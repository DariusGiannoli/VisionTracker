...
async def _run(serial_port: str, ws_host: str, ws_port: int, ws_path: str,
               update_hz: float = 20.0, smoothing: float = 0.8, deadzone_deg: float = 2.0):
    ...
    bus = FeetechMotorsBus(port=serial_port, motors=motors, calibration=None)
    bus.connect()
    ...
    for _ in range(int(3*update_hz)):
        ...
        # pitch
        v = -R_rel[2,0]; v = float(max(-1.0, min(1.0, v)))
    ...
    try:
        ...
        for _ in range(40):
            for name in pos:
                ...
                try: bus.write("Goal_Position", name, pos[name])
                except RuntimeError: pass
            time.sleep(0.03)
        bus.disconnect()
        await client.close()

def run_teleop(ws_host: str, serial_port: str, ws_port: int = 8211, ws_path: str = "/stream"):
    asyncio.run(_run(serial_port=serial_port, ws_host=ws_host, ws_port=ws_port, ws_path=ws_path))
